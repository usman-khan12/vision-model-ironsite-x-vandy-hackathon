...
class ObjectPermanenceModule(nn.Module):
    ...
    def __init__(
        ...
        self.object_predictor = ObjectPredictor(
            feature_dim, hidden_dim, use_transformer=use_transformer_predictor
        )
        self.percept_gate_controller = PerceptGateController(
            feature_dim, num_objects, hidden_dim
        )
    ...
    def forward(
        ...
        """
        batch_size = (
            current_model_outputs.shape[0]
            if isinstance(current_model_outputs, torch.Tensor)
            else current_model_outputs[0].shape[0]
        )
        device = (
            current_model_outputs.device
            if isinstance(current_model_outputs, torch.Tensor)
            else current_model_outputs[0].device
        )
        ...
        if self.feature_extractor is not None:
            current_features, object_positions, detection_confidence = (
                self.feature_extractor(current_model_outputs)
            )
        else:
            ...
            if isinstance(current_model_outputs, torch.Tensor):
                if len(current_model_outputs.shape) == 2:
                    ...
                    current_features = current_model_outputs.unsqueeze(1).expand(
                        -1, self.num_objects, -1
                    )
                else:
                    current_features = current_model_outputs[:, : self.num_objects, :]
            else:
                # Assume tuple/list of tensors
                current_features = current_model_outputs[0][:, : self.num_objects, :]

            object_positions = torch.zeros(
                batch_size, self.num_objects, 2, device=device
            )
            detection_confidence = torch.ones(
                batch_size, self.num_objects, device=device
            )
        ...
        if self.use_spatial_grid and self.spatial_grid is not None:
            ...
            object_ids, grid_match_confidence = self.spatial_grid.reidentify_object(
                current_features, object_positions
            )
        ...
        predicted_features, prediction_confidence = self.object_predictor(
            current_features, previous_features, temporal_context=velocity
        )
        ...
        if self.use_spatial_grid and self.spatial_grid is not None:
            ...
            position_velocities = (
                object_positions - self.tracker.previous_positions
                if self.tracker.previous_positions is not None
                else None
            )
            ...
            # Enhance predictions for occluded objects using grid features
            # at their peak occupancy locations (vectorised over batch).
            occluded_mask = occlusion_factors.mean(dim=0) > 0.5  # [num_objects]
            if occluded_mask.any() and self.spatial_grid.grid_state is not None:
                sg = self.spatial_grid
                # occupancy_maps: [B, N, H, W]
                flat_occ = occupancy_maps.view(batch_size, self.num_objects, -1)
                peak_idx = flat_occ.argmax(dim=-1)  # [B, N]
                peak_w = (peak_idx % sg.grid_width).clamp(0, sg.grid_width - 1)
                peak_h = (peak_idx // sg.grid_width).long().clamp(0, sg.grid_height - 1)
                ...
                o_idx = (
                    torch.arange(self.num_objects, device=device)
                    .unsqueeze(0)
                    .expand_as(peak_h)
                )
                grid_feat = sg.grid_state[b_idx, peak_h, peak_w, o_idx, :]  # [B, N, D]

                mask = occluded_mask.unsqueeze(0).unsqueeze(-1)  # [1, N, 1]
                predicted_features = torch.where(
                    mask, 0.5 * predicted_features + 0.5 * grid_feat, predicted_features
                )
