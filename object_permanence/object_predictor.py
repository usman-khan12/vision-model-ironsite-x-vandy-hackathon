...
class ObjectPredictor(nn.Module):
    ...
    def __init__(
        self, feature_dim, hidden_dim=256, num_layers=2, use_transformer=False
    ):
        """
        Args:
    ...
    def forward(self, current_features, previous_features=None, temporal_context=None):
        ...
        if temporal_context is not None:
            # Truncate or pad temporal context to match feature_dim
            ctx_dim = temporal_context.shape[-1]
            if ctx_dim > feature_dim:
                temporal_context = temporal_context[..., :feature_dim]
            elif ctx_dim < feature_dim:
                pad = torch.zeros(
                ...
                temporal_context = torch.cat([temporal_context, pad], dim=-1)
        ...
        if self.use_transformer:
            ...
            predicted_features = self.predict_head(mlp_out) + self.residual(
                input_features
            )
        ...
        if previous_features is not None:
            ...
            feature_stability = 1.0 - torch.mean(
                torch.abs(current_features - previous_features), dim=-1
            )
            confidence = torch.clamp(feature_stability, 0, 1)
        else:
            # Default confidence
            confidence = torch.ones(
                batch_size, num_objects, device=current_features.device
            )
